...
void PhoneBook::welcomeMessage(void)
{
    ...
    std::cout << BOLD_WHITE << "You can use the following commands:" << std::endl;
...
void PhoneBook::_printSearchTableInfo(int index, Contact contact)
{
    ...
    std::cout << std::setw(10) << _truncateString(contact.getValue("firstName")) << "|";
    std::cout << std::setw(10) << _truncateString(contact.getValue("lastName")) << "|";
    std::cout << std::setw(10) << _truncateString(contact.getValue("nickname")) << "|";
    std::cout << std::endl;
}

/**
 * @brief prints the contact info of the contact chosen by the user to the console.
...
void PhoneBook::printContactInfo(Contact info)
{
    ...
    std::cout << CYAN << "First name: " << RESET << info.getValue("firstName") << std::endl;
    std::cout << CYAN << "Last name: " << RESET << info.getValue("lastName") << std::endl;
    std::cout << CYAN << "Nickname: " << RESET << info.getValue("nickname") << std::endl;
    std::cout << CYAN << "Phone number: " << RESET << info.getValue("phoneNumber") << std::endl;
    std::cout << CYAN << "Darkest secret: " << RESET << info.getValue("darkestSecret") << std::endl;
}
...
void PhoneBook::_getInfo(std::string field, Contact& info)
{
    ...
    while (!str.length()) {
        ...
        if (str.length() == 0)
            std::cout << RED << "Ops! You must enter the contact's " + field + "!"
                << RESET << std::endl;
    }
